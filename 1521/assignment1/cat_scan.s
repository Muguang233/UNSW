...
print_welcome__body:
	li	$v0, 4
	la	$a0, str_print_welcome_1	#printf("===============================================================\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_2	#printf("                     Welcome to Cat Scan!\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_3	# printf("===============================================================\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_4	#printf("┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_5#printf("│  /\\_/\\  │  │  /\\_/\\  │  │  /\\_/\\  │  │  /\\_/\\  │  │  /\\_/\\  │\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_6	#printf("│ ( o.o ) │  │ ( ^_^ ) │  │ ( -.- ) │  │ ( uwu ) │  │ ( z_z ) │\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_7#printf("│ /     \\ │  │ /     \\ │  │ /     \\ │  │ /     \\ │  │ /     \\ │\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_8	#printf("└─────────┘  └─────────┘  └─────────┘  └─────────┘  └─────────┘\n");
	syscall
	li	$v0, 4
	la	$a0, str_print_welcome_9	#printf("===============================================================\n\n");
	syscall
...
game_setup:
	...
	# Uses:     [$ra, $a0, $a1, $a2, $a3, $t0, $t1, $t2, $t3]
	# Clobbers: [$a0, $a1, $a2, $a3, $t0, $t1, $t2, $t3]
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $a0		the first parameter use to call prompt_for_int
	#	- $a1		the second parameter use to call prompt_for_int
	#	- $a2		the third parameter use to call prompt_for_int
	#	- $a3		the fourth parameter use to call prompt_for_int
	#	- $t0		address of num_cats
	#	- $t1		value of height
	#	- $t2		value of width
	#	- $t3		new val of num_cats
	#
	# Structure:        <-- FILL THIS OUT!
	#   game_setup
	#   -> [prologue]
...
game_setup__body:
	...
	li	$a2, RNG_MAX_SEED
	la	$a3, random_number
	jal	prompt_for_int				#prompt_for_int("a random seed", RNG_MIN_SEED, RNG_MAX_SEED, &random_number);

	la	$a0, str_game_setup_2
	li	$a1, MIN_BOX_SIZE
	li	$a2, MAX_BOX_SIZE
	la	$a3, height
	jal prompt_for_int				#prompt_for_int("a height", MIN_BOX_SIZE, MAX_BOX_SIZE, &height);

	la	$a0, str_game_setup_3
	li	$a1, MIN_BOX_SIZE
	li	$a2, MAX_BOX_SIZE
	la	$a3, width
	jal prompt_for_int				#prompt_for_int("a width ", MIN_BOX_SIZE, MAX_BOX_SIZE, &width);

	la	$t0, num_cats				
	lw	$t1, height
	lw	$t2, width
	mul	$t3, $t1, $t2
	div	$t3, $t3, AREA_PER_CAT
	sw	$t3, ($t0)					#num_cats = (height * width) / AREA_PER_CAT;

	la	$t0, num_cats_found
	sw	$0, ($t0)					#num_cats_found = 0;

	jal	initialise_ball_of_string	#initialise_ball_of_string();

	jal	initialise_ball_paths		#initialise_ball_paths();

	jal initialise_box				#initialise_box();

	lw	$t0, num_cats
	mul	$t0, $t0, POINTS_PER_CAT
	la	$t1, score
	sw	$t0, ($t1)					#score = num_cats * POINTS_PER_CAT;
	...
	div	$t0, $t0, $t3
	la	$t1, cost_per_roll
	sw	$t0, ($t1)					#cost_per_roll = score / (width + height);

	la	$t0, is_cheat_mode
	li	$t1, FALSE
	sw	$t1, ($t0)					#is_cheat_mode = FALSE;

	la	$t0, is_auto_print
	li	$t1, TRUE
	sw	$t1, ($t0)					#is_auto_print = TRUE;

	la	$t0, game_state
	li	$t1, STATE_PLAYING
	sw	$t1, ($t0)					#game_state = STATE_PLAYING;
...
prompt_for_int__body:
	li	$t0, FALSE					#int is_valid_input = FALSE;
	li	$s0, 0						#int input
	move $s1, $a0
loop_cond:
	bnez $t0, loop_end
...
if_cond:
	blt	$s0, $a1, if_true			#input < min
	bgt	$s0, $a2, if_true			#input > max
	li	$t0, TRUE					#else { is_valid_input = TRUE; }
	b	loop_cond					#goto loop_cond
if_true:
	li	$v0, 4
	la	$a0, str_prompt_for_int_err_1
	syscall							# printf("Bad Input: value should be between
	li	$v0, 1
	move	$a0, $a1
	syscall							# %d min
	li	$v0, 4
...
initialise_ball_of_string__body:
	li	$t0, BALL_INIT					
	la	$t1, ball_of_string				
	sw	$t0, 0($t1)						#ball_of_string.start = BALL_INIT;
	sw	$t0, OFFSET_BALL_X($t1)			#ball_of_string.x = BALL_INIT;
	sw	$t0, OFFSET_BALL_Y($t1)			#ball_of_string.y = BALL_INIT;
	sw	$t0, OFFSET_BALL_DX($t1)		#ball_of_string.dx = BALL_INIT;
	sw	$t0, OFFSET_BALL_DY($t1)		#ball_of_string.dy = BALL_INIT;
	li	$t0, BALL_STATE_STOPPED			
	sw	$t0, OFFSET_BALL_STATE($t1)		#ball_of_string.state = BALL_STATE_STOPPED;
...
initialise_ball_paths:
	...
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $t0			address of ball_paths
	#	- $t1			int i
	#	- $t2			address for each offset
	#	- $t3			PATH_ENTRY_NOT_SET, PATH_EXIT_NOT_SET, PATH_STATE_NOT_CHECKED
	#
	# Structure:        <-- FILL THIS OUT!
	#   initialise_ball_paths
	#   -> [prologue]
...
initialise_ball_paths__body:
	...
	li	$t1, 0							#int i = 0			
for_cond:
	bge	$t1, MAX_PATHS, for_end			#for (int i = 0; i < MAX_PATHS; i++) 
	
for_body:
	mul	$t2, $t1, SIZEOF_BALL_PATH				#
	add	$t2, $t2, $t0
	li	$t3, PATH_ENTRY_NOT_SET			
	sw	$t3, OFFSET_BALL_PATH_ENTRY($t2)		#ball_paths[i].entry = PATH_ENTRY_NOT_SET;

	li	$t3, PATH_EXIT_NOT_SET
	sw	$t3, OFFSET_BALL_PATH_EXIT($t2)						#ball_paths[i].exit = PATH_EXIT_NOT_SET;

	li	$t3, PATH_STATE_NOT_CHECKED
	sw	$t3, OFFSET_BALL_PATH_STATE($t2)						#ball_paths[i].state = PATH_STATE_NOT_CHECKED;
	addi	$t1, $t1, 1					#i += 1
	b	for_cond
for_end:
...
row_cond:
	bge	$t0, MAX_BOX_SIZE, row_end		#if(i >= MAX_PATHS) goto row_end
	li	$t1, 0							#int col = 0
col_cond:
	bge	$t1, MAX_BOX_SIZE, col_end		#if(j >= MAX_PATHS) goto col_end
body:
	mul	$t4, $t0, MAX_BOX_SIZE			# row * MAX_BOX_SIZE
	add	$t4, $t4, $t1					# row * MAX_BOX_SIZE + col
	mul	$t4, $t4, SIZEOF_INT			# (row * MAX_BOX_SIZE + col) * SIZEOF_INT
	add	$t4, $t4, $t2					# [(row * MAX_BOX_SIZE + col) * SIZEOF_INT] + base addr box

	li	$t5, FALSE
	sw	$t5, OFFSET_BOX_CONTAINS_CAT($t4)						#box.contains_cat[row][col] = FALSE;
	sw	$t5, OFFSET_BOX_GUESSED($t4)						#box.guessed[row][col] = FALSE;
	addi	$t1, $t1, 1					#col += 1
	b		col_cond					# branch to col_cond
...
print_stats:
	...
	#   - $t0			score, num_cats
	#	- $t1			num_guesses_left
	#	- $t2			cost_per_roll, num_rolls_left, remain_cat
	#	- $a0			syscall register
	#
	# Structure:        <-- FILL THIS OUT!
	#   print_stats
...
print_stats__body:
	lw	$t0, score							#int score
	div	$t1, $t0, POINT_LOSS_BAD_GUESS		#int num_guesses_left

	li	$v0, 4
	la	$a0, str_print_stats_1
	syscall									#printf("You have 
	...
	li	$v0, 1
	move	$a0, $t1
	syscall									#%d, num_guesses_left

print_stats_if_cond:
	bne	$t1, 1, print_stats_if_false
	li	$v0, 4
	la	$a0, str_print_stats_3
	syscall									#printf(" guess / ");
	b	print_stats_if_end
print_stats_if_false:
	li	$v0, 4
	la	$a0, str_print_stats_4
	syscall									#printf(" guesses / ");
print_stats_if_end:
	lw	$t2, cost_per_roll
	div	$t2, $t0, $t2						#int num_rolls_left = score / cost_per_roll;

	li	$v0, 1
	move	$a0, $t2
	syscall									#printf("%d, num_rolls_left

	li	$v0, 4
	la	$a0, str_print_stats_5
	syscall									# roll(s) left, with "

	lw	$t0, num_cats
	lw	$t1, num_cats_found
	sub	$t2, $t0, $t1						#int remain_cat = num_cats - num_cats_found

	li	$v0, 1
	move	$a0, $t2
	syscall									#printf("%d, remain_cat

	li	$v0, 4
	la	$a0, str_print_stats_6
	syscall									# cat(s) remaining.\n"
...
game_loop__body:
	...
	lw	$a0, num_cats				
	jal	spawn_cats					#spawn_cats(num_cats);
...
game_loop_while_cond:
	lw	$s0, game_state
	bne	$s0, STATE_PLAYING, game_loop_while_end
	jal	process_command
	b	game_loop_while_cond
game_loop_while_end:

game_loop_if_cond:
	bne	$s0, STATE_WIN, game_loop_if_false	#if(game_state == STATE_LOSE) goto game_loop_if_false
	
	li	$v0, 4
	la	$a0, str_game_loop_1
	syscall									#printf("All cats found.\n");

	jal	print_smiling_cat					#print_smiling_cat();

	li	$v0, 4
	la	$a0, str_game_loop_2
	syscall									#printf("You win! Final Score: 
	...
	b	game_loop_if_end
game_loop_if_false:
	bne	$s0, STATE_LOSE, game_loop_if_end
	jal	print_crying_cat					#print_crying_cat();
...
is_cat_at_position:
	# Subset:   2
	#
	# Frame:    [$s0, $s1]   <-- FILL THESE OUT!
	# Uses:     [$s0, $s1, $t0, $t1, $t2, $v0]
	# Clobbers: [...]
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $t0			base addr box
	#   - $t1			addr box
	#   - $t2			addr box.contains_cat[row][col]
	#   - $s0			row
	#   - $s1			col
	#	- $v0			return is_position_in_box(row, col)
	#
	# Structure:        <-- FILL THIS OUT!
	#   is_cat_at_position
	#   -> [prologue]
...
is_cat_at_position__body:
	move	$s0, $a0						#int row
	move	$s1, $a1						#int col
	jal	is_position_in_box					#return is_position_in_box(row, col)
	beqz	$v0, is_cat_at_position_false	#if(!is_position_in_box(row, col)) goto is_cat_at_position_false
	la	$t0, box							
	mul	$t1, $s0, MAX_BOX_SIZE				#row * MAX_BOX_SIZE
	add	$t1, $t1, $s1						#row * MAX_BOX_SIZE + col
	mul	$t1, $t1, SIZEOF_INT				#(row * MAX_BOX_SIZE + col) * SIZEOF_INT
	add	$t1, $t1, $t0						#(row * MAX_BOX_SIZE + col) * SIZEOF_INT + base addr box
	lw	$t2, ($t1)							#box.contains_cat[row][col]
	bne	$t2, TRUE, is_cat_at_position_false	#if(!box.contains_cat[row][col]) goto is_cat_at_position_false
...
do_move_ball_forward__body:
	...
	lw	$t0, OFFSET_BALL_X($s0)		#$t0 = ball->x
	lw	$t1, OFFSET_BALL_DX($s0)	#$t1 = ball->dx
	lw	$t2, OFFSET_BALL_Y($s0)		#$t2 = ball->y
	lw	$t3, OFFSET_BALL_DY($s0)	#$t3 = ball->dy
	add	$t0, $t0, $t1
	sw	$t0, OFFSET_BALL_X($s0)		#ball->x = ball->x + ball->dx
	add	$t2, $t2, $t3
	sw	$t2, OFFSET_BALL_Y($s0)		#ball->y = ball->y + ball->dy
...
do_turn_ball:
	# Subset:   2
	#
	# Frame:    [$s0, $s1, $ra]   <-- FILL THESE OUT!
	# Uses:     [$s0, $s1, $t0, $t1, $a0, $a1]
	# Clobbers: [$a0]
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $s0			struct ball_of_string *ball
	...
	#   -> [prologue]
	#       -> body
	#			->do_turn_ball_if_cond
	#			->do_turn_ball_if_body
	#			->do_turn_ball_if_false
	#			->do_turn_ball_if_end
	#   -> [epilogue]
...
do_turn_ball__body:
	move	$s0, $a0							#struct ball_of_string *ball
	move	$s1, $a1							#int turn_direction
do_turn_ball_if_cond:
	beq	$s1, TURN_RIGHT, do_turn_ball_if_false	#if(turn_direction == TURN_RIGHT) goto do_turn_ball_if_false
	
do_turn_ball_if_body:
	lw	$t0, OFFSET_BALL_DX($s0)				#int dx = ball->dx;
	lw	$t1, OFFSET_BALL_DY($s0)				#int ball-dy
	sw	$t1, OFFSET_BALL_DX($s0)				#ball->dx = ball->dy;
	nor	$t0, $t0, $zero
	addi	$t0, $t0, 1							#dx = -dx
	sw	$t0, OFFSET_BALL_DY($s0)				#ball->dy = -dx;
	b	do_turn_ball_if_end						#goto do_turn_ball_if_end
do_turn_ball_if_false:
	move	$a0, $s0							#ball
	li	$a1, TURN_LEFT							#TURN_LEFT
...
spawn_cats:
	# Subset:   2
	#
	# Frame:    [$s0, $s1, $s2, $s3]   <-- FILL THESE OUT!
	# Uses:     [$s0, $s1, $s2, $s3, $t0, $t1, $t2, $t3]
	# Clobbers: [...]
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $s0			amount_of_cats
	...
	#   - $s3			int i
	#   - $t0			base addr box
	#   - $t1			address of box.contains_cat[cat_row][cat_col]
	#   - $t2			box.contains_cat[cat_row][cat_col]
	#   - $t3			TRUE
	#
	# Structure:        <-- FILL THIS OUT!
	#   spawn_cats
	#   -> [prologue]
	#       -> body
	#			->spawn_cats__body_for_cond
	#			->spawn_cats__body_for_body
	#				->spawn_cats__body_loop_cond
	#				->spawn_cats__body_loop_body
	#				->spawn_cats__body_loop_end
	#			->spawn_cats__body_for_end
	#   -> [epilogue]
...
spawn_cats__body:
	...
	li	$s3, 0					#int i = 0
spawn_cats__body_for_cond:
	bge	$s3, $s0, spawn_cats__body_for_end	#if(i>=amount_of_cats) goto spawn_cats__body_for_end
spawn_cats__body_for_body:
	li	$a0, 0								
	lw	$a1, height
	sub	$a1, $a1, 1
	jal	random_in_range
	move	$s1, $v0						#int cat_row = random_in_range(0, height - 1);

	li	$a0, 0
	lw	$a1, width
	sub	$a1, $a1, 1
	jal	random_in_range
	move	$s2, $v0						#int cat_col = random_in_range(0, width - 1);
spawn_cats__body_loop_cond:
	la	$t0, box							#base addr box
	mul	$t1, $s1, MAX_BOX_SIZE				#cat_row * MAX_BOX_SIZE
	add	$t1, $t1, $s2						#cat_row * MAX_BOX_SIZE + cat_col
	mul	$t1, $t1, SIZEOF_INT				#(cat_row * MAX_BOX_SIZE + cat_col) * SIZEOF_INT
	add	$t1, $t1, $t0						#(cat_row * MAX_BOX_SIZE + cat_col) * SIZEOF_INT + base addr box
	lw	$t2, ($t1)							#box.contains_cat[cat_row][cat_col]

	bne	$t2, TRUE, spawn_cats__body_loop_end	#if(!box.contains_cat[cat_row][cat_col]) goto spawn_cats__body_loop_end
spawn_cats__body_loop_body:
	li	$a0, 0
	lw	$a1, height
	sub	$a1, $a1, 1
	jal	random_in_range
	move	$s1, $v0						#cat_row = random_in_range(0, height - 1);
	...
	sub	$a1, $a1, 1
	jal	random_in_range
	move	$s2, $v0						#cat_col = random_in_range(0, width - 1);
...
spawn_cats__body_loop_end:
	...
	mul	$t1, $t1, SIZEOF_INT				#(cat_row * MAX_BOX_SIZE + cat_col) * SIZEOF_INT
	add	$t1, $t1, $t0						#(cat_row * MAX_BOX_SIZE + cat_col) * SIZEOF_INT + base addr box
	sw	$t3, ($t1)							#box.contains_cat[cat_row][cat_col] = TRUE;
	addi	$s3, $s3, 1						#i += 1
	b	spawn_cats__body_for_cond			#goto spawn_cats__body_for_cond
spawn_cats__body_for_end:
...
guess_cat_location:
	...
	#
	# Frame:    [$s0, $s1]   <-- FILL THESE OUT!
	# Uses:     [$s0, $s1, $t0, $t1, $t2, $t3, $t4, $t5, $a0, $a1, $a2, $a3]
	# Clobbers: [$t0, $t1, $t2, $t3, $t4, $t5]
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $s0			int row
	#   - $s1			int col
	#   - $t0			height, base addr box, addr score, num_cats_found
	#   - $t1			guess_y, is_auto_print, num_cats, score, addr box
	#   - $t2			guess_x, box.contains_cat[row][col], STATE_WIN, game_state address
	#   - $t3			base addr box, score, STATE_LOSE, base addr game_state
	#   - $t4			addr box.guessed[row][col], num_cats_found, score
	#   - $t5			box.guessed[row][col], TRUE
	#   - $a0			syscall, param1
	#   - $a1			parameter to call function prompt_for_int
	#   - $a2			parameter to call function prompt_for_int
	#   - $a3			parameter to call function prompt_for_int
	#
	# Structure:        <-- FILL THIS OUT!
	#   guess_cat_location
	#   -> [prologue]
	#       -> body
	#			->guess_cat_location_loop
	#				->guess_cat_location_if_in_loop
	#			->guess_cat_location_loop_end
	#			->guess_cat_location_is_auto_print
	#			->guess_cat_location_box_contains_cat
	#				->guess_cat_location_if_all_found
	#				->guess_cat_location_if_all_found_false
	#			->guess_cat_location_box_not_contains_cat
	#				->guess_cat_location_box_if_lose
	#				->guess_cat_location_box_if_lose_end
	#		->end
	#   -> [epilogue]
...
guess_cat_location_loop:
	...
	lw	$a2, height
	la	$a3, guess_y
	jal	prompt_for_int						#prompt_for_int("a row edge number", 1, height, &guess_y);

	la	$a0, str_guess_cat_location_2
	lw	$a1, height							
	addi	$a1, $a1, 1						#height + 1
	lw	$t0, height
	lw	$a2, width
	add	$a2, $a2, $t0						#height + width
	la	$a3, guess_x
	jal	prompt_for_int					#prompt_for_int("a col edge number", height + 1, height + width, &guess_x);

	lw	$t1, guess_y
	sub	$t1, $t1, 1							
	move	$s0, $t1						#row = guess_y - 1;

	lw	$t0, height
	lw	$t2, guess_x
	addi	$t0, $t0, 1						#height + 1
	sub	$t2, $t2, $t0						#guess_x - (height + 1)
	move	$s1, $t2						#col = guess_x - (height + 1);
guess_cat_location_if_in_loop:
	la	$t3, box
	mul	$t4, $s0, MAX_BOX_SIZE				#row * MAX_BOX_SIZE
	add	$t4, $t4, $s1						#row * MAX_BOX_SIZE + col
	mul	$t4, $t4, SIZEOF_INT				#(row * MAX_BOX_SIZE + col) * SIZEOF_INT
	add	$t4, $t4, $t3						#(row * MAX_BOX_SIZE + col) * SIZEOF_INT + box
	lw	$t5, OFFSET_BOX_GUESSED($t4)		#box.guessed[row][col]
	beq	$t5, FALSE, guess_cat_location_loop_end	#if(box.guessed[row][col]== FALSE) goto guess_cat_location_loop_end

	li	$v0, 4
	la	$a0, str_guess_cat_location_3
	syscall								#printf("Bad Input: This location has already been guessed. Try again!\n");
	b	guess_cat_location_loop				#goto guess_cat_location_loop
guess_cat_location_loop_end:	
	la	$t3, box							#
	mul	$t4, $s0, MAX_BOX_SIZE				#row * MAX_BOX_SIZE
	add	$t4, $t4, $s1						#row * MAX_BOX_SIZE + col
	...
	add	$t4, $t4, $t3						#(row * MAX_BOX_SIZE + col) * SIZEOF_INT + box
	li	$t5, TRUE							
	sw	$t5, OFFSET_BOX_GUESSED($t4)		#box.guessed[row][col] = TRUE;
guess_cat_location_is_auto_print:
	lw	$t1, is_auto_print
	bne	$t1, TRUE, guess_cat_location_box_contains_cat	#if(!is_auto_print)) goto guess_cat_location_box_contains_cat
...
guess_cat_location_box_contains_cat:
	...
	add	$t1, $t1, $s1										#row * MAX_BOX_SIZE + col
	mul	$t1, $t1, SIZEOF_INT								#(row * MAX_BOX_SIZE + col) * SIZEOF_INT
	add	$t1, $t1, $t0										#(row * MAX_BOX_SIZE + col) * SIZEOF_INT + box
	lw	$t2, ($t1)
	bne	$t2, TRUE, guess_cat_location_box_not_contains_cat		#if (box.contains_cat[row][col])

	li	$v0, 4
	la	$a0, str_guess_cat_location_4
	syscall														#printf("Good Guess! [+

	li	$v0, 1
	li	$a0, POINT_GAIN_GOOD_GUESS
	syscall														#%d, POINT_GAIN_GOOD_GUESS
	...
	la	$a0, cat_name
	jal	generate_cat_name										#generate_cat_name(cat_name);

	li	$v0, 4
	la	$a0, str_guess_cat_location_6
	syscall														#printf("You found \"
	...
	la	$t4, score												#addr score
	lw	$t3, score												#score		
	addi	$t3, $t3, POINT_GAIN_GOOD_GUESS						#score = score - POINT_GAIN_GOOD_GUESS		
	sw	$t3, ($t4)												#score = score - POINT_GAIN_GOOD_GUESS
	la	$t4, num_cats_found
	lw	$t3, num_cats_found
	addi	$t3, $t3, 1
	sw	$t3, ($t4)												# num_cats_found += 1
guess_cat_location_if_all_found:
	lw	$t0, num_cats_found										#
	lw	$t1, num_cats
	bne	$t0, $t1, guess_cat_location_if_all_found_false		#if (num_cats_found != num_cats) 
	li	$t2, STATE_WIN											
	la	$t3, game_state
	sw	$t2, ($t3)												#game_state = STATE_WIN;
	b	guess_cat_location_end									#goto guess_cat_location_end
guess_cat_location_if_all_found_false:
...
guess_cat_location_box_not_contains_cat:
	...
	syscall														#printf("Bad Guess [-

	li	$v0, 1
	li	$a0, POINT_LOSS_BAD_GUESS
	syscall														#%d, POINT_LOSS_BAD_GUESS
	...
	la	$t0, score
	lw	$t1, score
	sub	$t1, $t1, POINT_LOSS_BAD_GUESS
	sw	$t1, ($t0)												#score = score - POINT_LOSS_BAD_GUESS
guess_cat_location_box_if_lose:
	bge	$t1, POINT_LOSS_BAD_GUESS, guess_cat_location_box_if_lose_end	#if (score >= POINT_LOSS_BAD_GUESS)
	la	$t2, game_state
	li	$t3, STATE_LOSE
	sw	$t3, ($t2)												#game_state = STATE_LOSE;
guess_cat_location_box_if_lose_end:
	jal	print_stats												#print_stats();
guess_cat_location_end:
...
handle_ball_movement:
	...
	# Uses:     [$s0, $s1, $s2, $s3, $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7, $a0, $a1, $a2, $v0]
	# Clobbers: [$t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7, $a0, $a1, $a2, $v0]
	#
	# Locals:           <-- FILL THIS OUT!
	#   - $s0			struct ball *ball
	#   - $s1			struct ball_path *paths
	#   - $s2			int interaction
	#   - $s3			struct ball_path *path
	#   - $t0			ball->start, save return value from edge_number_from_position
	#   - $t1			PATH_STATE_ABSORBED, BALL_STATE_STOPPED, PATH_STATE_REFLECTED, BALL_STATE_STOPPED, 
	#   - $t2			path
	#   - $t3			path->exit
	#   - $t4			path->entry
	#   - $t5			PATH_STATE_CHECKED
	#   - $t6			ball->x
	#   - $t7			ball->y
	#   - $a0			parameter to call function do_turn_ball, do_move_ball_forward, is_position_in_box, 
	#   - $a1			parameter to call function do_turn_ball, do_move_ball_forward, is_position_in_box, 
	#   - $a2			parameter to call function do_turn_ball, do_move_ball_forward, is_position_in_box, 
	#   - $v0			return value from function edge_number_from_position
	#
	# Structure:        <-- FILL THIS OUT!
	#   handle_ball_movement
	#   -> [prologue]
	#       -> body
	#			->handle_ball_movement_if_first
	#			->handle_ball_movement_if_first_false
	#			->handle_ball_movement_if_second
	#			->handle_ball_movement_if_second_false
	#			->handle_ball_movement_if_second_end
	#			->handle_ball_movement_if_third
	#		->end
	#   -> [epilogue]
...
handle_ball_movement__body:
	...
	sub	$t0, $t0, 1
	mul	$t1, $t0, SIZEOF_BALL_PATH
	add	$t1, $t1, $s1
	move	$s3, $t1							#struct ball_path *path = &paths[ball->start - 1];
handle_ball_movement_if_first:
	bne	$s2, INTERACTION_ABSORB, handle_ball_movement_if_first_false		#if (interaction != INTERACTION_ABSORB) 

	lw	$t0, ($s0)
	sw	$t0, OFFSET_BALL_PATH_EXIT($s3)			#path->exit = ball->start;

	li	$t1, PATH_STATE_ABSORBED
	sw	$t1, OFFSET_BALL_PATH_STATE($s3)		#path->state = PATH_STATE_ABSORBED;

	li	$t1, BALL_STATE_STOPPED
	sw	$t1, OFFSET_BALL_STATE($s0)				#ball->state = BALL_STATE_STOPPED;

	b	handle_ball_movement_end				#return;
handle_ball_movement_if_first_false:
	bne	$s2, INTERACTION_REFLECT, handle_ball_movement_if_second		#else if (interaction != INTERACTION_REFLECT) 

	lw	$t0, ($s0)
	sw	$t0, OFFSET_BALL_PATH_EXIT($s3)			#path->exit = ball->start;

	li	$t1, PATH_STATE_REFLECTED
	sw	$t1, OFFSET_BALL_PATH_STATE($s3)		#path->state = PATH_STATE_REFLECTED;
...
handle_ball_movement_if_second:
	bne	$s2, INTERACTION_DEFLECT_LEFT, handle_ball_movement_if_second_false
	move	$a0, $s0
	li	$a1, TURN_LEFT
	jal	do_turn_ball
	b	handle_ball_movement_if_second_end
handle_ball_movement_if_second_false:
	bne	$s2, INTERACTION_DEFLECT_RIGHT, handle_ball_movement_if_second_end
	move	$a0, $s0
	li	$a1, TURN_RIGHT
	jal	do_turn_ball
handle_ball_movement_if_second_end:
...
handle_ball_movement_if_third:
	lw	$t6, OFFSET_BALL_Y($s0)
	move	$a0, $t6
	lw	$t7, OFFSET_BALL_X($s0)
	move	$a1, $t7
	jal	is_position_in_box
	move	$t0, $v0
	bnez	$t0, handle_ball_movement_end

	li	$t1, BALL_STATE_STOPPED
	sw	$t1, OFFSET_BALL_STATE($s0)
	...
	li	$t1, PATH_STATE_CHECKED
	sw	$t1, OFFSET_BALL_PATH_STATE($s3)				#path->state = PATH_STATE_CHECKED;

	lw	$t2, OFFSET_BALL_PATH_EXIT($s3)
	sub	$t2, $t2, 1
	mul	$t2, $t2, SIZEOF_BALL_PATH
	add	$t2, $t2, $s1
	lw	$t3, OFFSET_BALL_PATH_EXIT($s3)
	sw	$t3, OFFSET_BALL_PATH_ENTRY($t2)

	lw	$t4, OFFSET_BALL_PATH_ENTRY($s3)
	sw	$t4, OFFSET_BALL_PATH_EXIT($t2)

	li	$t5, PATH_STATE_CHECKED
	sw	$t5, OFFSET_BALL_PATH_STATE($t2)
handle_ball_movement_end:
