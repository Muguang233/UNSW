...
int bstNumLeaves(struct node *t) {
	...
	if(!bstNumLeaves(t->left) && !bstNumLeaves(t->right)){
		return 1;
	}else{
...
int bstClosest(struct node *t, int val) {
	...
	int current = abs(t->value - val);
	int left_val = bstClosest(t->left,val);
	int left = abs(left_val-val);
	int right_val = bstClosest(t->right,val);
	int right = abs(right_val-val);
