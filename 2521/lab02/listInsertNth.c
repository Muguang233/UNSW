...
void insertNode(struct node *list, int index, int value);
...
void listInsertNth(struct list *list, int n, int value) {
	...
	if(n == 0){
		...
		insertNode(list->head, length-1, value);
	}else{
		insertNode(list->head, n-1, value);
	}
}
void insertNode(struct node *list, int index, int value){
	if(index != 0){
		insertNode(list->next, index-1, value);
	}else{
		struct node *new = newNode(value);
		new->next = list->next;
		list->next = new;
